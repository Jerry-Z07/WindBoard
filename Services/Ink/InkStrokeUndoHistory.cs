using System.Collections.Generic;
using WindBoard.Models.Ink;

namespace WindBoard.Services.Ink
{
    internal sealed class InkStrokeUndoHistory
    {
        private sealed class Delta
        {
            public HashSet<InkStrokeModel> Added { get; } = new HashSet<InkStrokeModel>();
            public HashSet<InkStrokeModel> Removed { get; } = new HashSet<InkStrokeModel>();

            public bool IsEmpty => Added.Count == 0 && Removed.Count == 0;

            public void RecordAdded(InkStrokeModel stroke)
            {
                if (!Removed.Remove(stroke))
                {
                    Added.Add(stroke);
                }
            }

            public void RecordRemoved(InkStrokeModel stroke)
            {
                if (!Added.Remove(stroke))
                {
                    Removed.Add(stroke);
                }
            }

            public void Record(IReadOnlyList<InkStrokeModel> added, IReadOnlyList<InkStrokeModel> removed)
            {
                if (added != null)
                {
                    for (int i = 0; i < added.Count; i++)
                    {
                        var s = added[i];
                        if (s != null) RecordAdded(s);
                    }
                }

                if (removed != null)
                {
                    for (int i = 0; i < removed.Count; i++)
                    {
                        var s = removed[i];
                        if (s != null) RecordRemoved(s);
                    }
                }
            }

            public void Undo(List<InkStrokeModel> strokes)
            {
                foreach (var s in Added)
                {
                    strokes.Remove(s);
                }

                foreach (var s in Removed)
                {
                    if (!strokes.Contains(s))
                    {
                        strokes.Add(s);
                    }
                }
            }

            public void Redo(List<InkStrokeModel> strokes)
            {
                foreach (var s in Removed)
                {
                    strokes.Remove(s);
                }

                foreach (var s in Added)
                {
                    if (!strokes.Contains(s))
                    {
                        strokes.Add(s);
                    }
                }
            }
        }

        private readonly Stack<Delta> _undo = new Stack<Delta>();
        private readonly Stack<Delta> _redo = new Stack<Delta>();
        private Delta? _current;
        private int _suspendCount;

        public bool CanUndo => _undo.Count > 0;
        public bool CanRedo => _redo.Count > 0;
        public bool IsInTransaction => _current != null;

        public void Begin()
        {
            if (_suspendCount > 0) return;
            _current ??= new Delta();
        }

        public void End()
        {
            if (_suspendCount > 0)
            {
                _current = null;
                return;
            }

            if (_current == null) return;

            if (!_current.IsEmpty)
            {
                _undo.Push(_current);
                _redo.Clear();
            }

            _current = null;
        }

        public void Cancel()
        {
            _current = null;
        }

        public void Clear()
        {
            _undo.Clear();
            _redo.Clear();
            _current = null;
        }

        public void Record(IReadOnlyList<InkStrokeModel> added, IReadOnlyList<InkStrokeModel> removed)
        {
            if (_suspendCount > 0) return;
            if (_current == null) return;
            _current.Record(added, removed);
        }

        public void Undo(List<InkStrokeModel> strokes)
        {
            if (_suspendCount > 0) return;
            if (_undo.Count == 0) return;

            var d = _undo.Pop();
            using (SuspendRecording())
            {
                d.Undo(strokes);
            }
            _redo.Push(d);
        }

        public void Redo(List<InkStrokeModel> strokes)
        {
            if (_suspendCount > 0) return;
            if (_redo.Count == 0) return;

            var d = _redo.Pop();
            using (SuspendRecording())
            {
                d.Redo(strokes);
            }
            _undo.Push(d);
        }

        public System.IDisposable SuspendRecording()
        {
            _suspendCount++;
            return new SuspendHandle(this);
        }

        private sealed class SuspendHandle : System.IDisposable
        {
            private InkStrokeUndoHistory? _owner;

            public SuspendHandle(InkStrokeUndoHistory owner) => _owner = owner;

            public void Dispose()
            {
                var o = _owner;
                if (o == null) return;
                _owner = null;
                o._suspendCount--;
                if (o._suspendCount < 0) o._suspendCount = 0;
            }
        }
    }
}


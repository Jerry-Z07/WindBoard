...
- [x] 手测回归清单（prd.md 验收 1-6 条）经用户验收放行（2026-09-09）：确认四形状绘制/预览/可见性、属性面板、工具栏入口与图标状态；期间发现并修复——Dock 注册缺失导致入口丢失、初始画笔色未同步导致形状"黑笔深底不可见"、D2D 跨工厂描边样式导致整帧不呈现、预览通道 Stroke 门控、图标选中态/主题取色/初始求值时序

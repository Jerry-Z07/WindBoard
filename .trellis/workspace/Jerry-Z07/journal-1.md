...
## Session 18: 形状创建后自动选中与尺寸厘米显示
<!-- trellis-session: v=2 fp=073058f963d7fdd1 -->

**Date**: 2026-09-19
**Task**: 形状创建后自动选中与尺寸厘米显示
**Branch**: `develop`

### Summary

实现形状提交后自动切到选择工具并选中新形状（工具经控制器事件在状态清理后把结果交给宿主，屏幕批注不订阅）；属性浮层的长度/宽高按固定换算 1 世界单位 = 1/96 英寸显示与编辑，域存储/渲染/导出/WBIX 不变；顺带修复既有缺陷：浮层切换选中目标时把上一个形状的字段值提交到新形状（程序化改边界期间屏蔽提交 + 换目标强制回读）；重做浮层输入框触控尺寸（44 高/200 宽，步进按钮 Compact→Inline 消除聚焦 Flyout 遮挡）。构建零告警，591 测试全绿；规范沉淀工具提交事件契约、域长度换算约定与浮层控件触控基准。

### Git Commits

| Hash | Message |
|------|---------|
| `6b49364` | feat: 形状创建后自动选中，尺寸按厘米显示与编辑 |
| `6b5bc7f` | feat: 重做形状属性浮层输入框的触控尺寸与步进按钮呈现 |
| `657d976` | docs(spec): 记录工具提交事件契约、域长度换算约定与浮层控件触控约定 |

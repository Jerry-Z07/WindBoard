...
{
    /// <summary>
    /// 擦除策略接口：输入为世界坐标下的“橡皮擦轨迹线段 + 半径”，实现可对文档执行整笔擦除或局部擦除（分段）等逻辑。
    /// </summary>
    internal interface IBoardEraser
    {
        ...
        /// <param name="radiusWorld">橡皮擦半径（世界坐标）。</param>
        /// <returns>若文档发生变化（删除/分割/替换笔迹）则返回 true。</returns>
        bool Erase(BoardDocument document, Vector2 fromWorld, Vector2 toWorld, float radiusWorld);
    }
}

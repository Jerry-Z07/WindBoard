...
namespace WindBoard.Settings.Pages
{
    public sealed partial class AboutSettingsPage
    {
        ...
        private StackPanel BuildUpdateSummaryPanel(
            AppUpdateCheckResult result,
            DownloadSourceId sourceForUrls,
            string releasePageUrl,
            Func<Task> closeCurrentDialogAsync)
        ...
        private void AppendPlainTextChangelog(StackPanel panel, UpdateResultDialogLayoutPlan layoutPlan)
        {
            if (string.IsNullOrWhiteSpace(layoutPlan.ChangelogMarkdown))
            {
                return;
            }
            ...
            panel.Children.Add(new ScrollViewer
            {
                ...
                {
                    Text = layoutPlan.ChangelogMarkdown,
        ...
        private UIElement BuildMarkdownChangelogContent(UpdateResultDialogLayoutPlan layoutPlan)
        {
            ...
            {
                return BuildChangelogPlaceholderTextBlock();
            }

            try
            {
                MarkdownViewDocument document = MarkdownViewDocumentBuilder.Build(layoutPlan.ChangelogMarkdown);
                if (document.Blocks.Count == 0)
                {
                    return BuildChangelogPlaceholderTextBlock();
                }

                return MarkdownViewRenderer.Build(document, url => _ = TryLaunchUrlAsync(url));
            }
            catch (Exception ex)
            {
                AppLog.Warn("Updates", "渲染更新内容 Markdown 失败，已降级为纯文本", ex);
        ...
        private static TextBlock BuildChangelogPlaceholderTextBlock()
        {
            return new TextBlock
            {
                Text = L10n.Get("Updates_Changelog_Empty"),
                Opacity = 0.7,
                TextWrapping = TextWrapping.WrapWholeWords,
            };
        }
            ...
            Func<Task> closeCurrentDialogAsync)
        {
            ...
            if (result.State != AppUpdateCheckState.UpdateAvailable || result.Assets.Recommended is null)
            {
                return;
            }
